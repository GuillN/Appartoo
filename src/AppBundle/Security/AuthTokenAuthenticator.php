...
class AuthTokenAuthenticator implements SimplePreAuthenticatorInterface, AuthenticationFailureHandlerInterface
{
    ...
    public function createToken(Request $request, $providerKey)
    {
        ...
        if (($request->getMethod() === "POST" || $request->getMethod() === "GET") && $this->httpUtils->checkRequestPath($request, $targetUrl)) {
            return;
        }

        if($request->getMethod() === "POST" && $this->httpUtils->checkRequestPath($request, $registerUrl)) {
            return;
        }
